...
public class MazeController : MonoBehaviour
{
   ...
   [SerializeField]
   private float MIN_DISTANCE_BETWEEN_SLOTS = 20f;


   void Awake()
   {
      slotMachinePrefabsInScene = FindObjectsByType<MachineController>(
   ...
   void Start()
   {
      ...
      {
         Debug.LogWarning("No Slot Machine prefabs found in the scene. Please ensure they are present in the scene.");
      }
      else
      {
         Debug.Log($"Found {slotMachinePrefabsInScene.Count} Slot Machine prefabs in the scene.");
      }
   }
   ...
   private void GenerateMaze()
   {
      ...
      GameData.Maze[GameData.MazeRows - 1, exit].WallTop = 3;
      // Create a new object with a box collider
      GameObject exitCollider = new GameObject("ExitCollider");
      exitCollider.AddComponent<BoxCollider>();
      //Set the collider to be a trigger
      exitCollider.GetComponent<BoxCollider>().isTrigger = true;
      //Make the object the size of a cell
      exitCollider.transform.localScale = new Vector3(GameData.CellSize * GameData.MazeRows, 1f, GameData.CellSize);
      // Set the position of the object to the position of the exit cell
      exitCollider.transform.position = new Vector3((GameData.MazeColumns * GameData.CellSize) / 2, 0, (GameData.MazeRows + 1) * GameData.CellSize);
      // Add a ExitController script component to the Object
      exitCollider.AddComponent<ExitController>();
      #endregion
   ...
   private void Create3DWalls()
   {
      ...
      for (int i = 0; i < GameData.MazeRows; i++)
      {
         for (int j = 0; j < GameData.MazeColumns; j++)
         {
            ...
            {
               GameObject prefab = GetWallPrefab(GameData.Maze[i, j].WallTop);
            ...
            {
               GameObject prefab = GetWallPrefab(GameData.Maze[i, j].WallRight);
            ...
            {
               GameObject prefab = GetWallPrefab(GameData.Maze[i, j].WallBottom);
            ...
            {
               GameObject prefab = GetWallPrefab(GameData.Maze[i, j].WallLeft);
   ...
   private GameObject GetWallPrefab(int wallType)
   {
      switch (wallType)
      {
         case 1:
            return Resources.Load<GameObject>("Prefabs/OuterWalls");
         case 2:
            return Resources.Load<GameObject>("Prefabs/InnerWall");
         case 3:
            return Resources.Load<GameObject>("Prefabs/FinishDoor");
         default:
            Debug.LogError($"Unknown wall type: {wallType}");
            return null;
      }
   }
   ...
   private void CreateOpenSpace()
   {
      ...
      List<GameObject> existingSlotMachines = GameObject.FindObjectsByType<MachineController>(FindObjectsSortMode.None).Select(sm => sm.gameObject).ToList();

      for (int k = 0; k < numberOfOpenSpaces; k++)
      {
         bool spaceAndSlotCreated = false;
         int attempts = 0;
         while (!spaceAndSlotCreated && attempts < 20)
         {
            int startRow = UnityEngine.Random.Range(2, GameData.MazeRows - 4);
            int startCol = UnityEngine.Random.Range(2, GameData.MazeColumns - 4);
            ...
            if (!overlaps)
            {
               ...
               if (!tooClose)
               {
                  ...
                  if (availablePrefabs.Count > 0)
                  {
                     ...
                     {
                        var prefabMC = selectedPrefab.GetComponent<MachineController>();

                        Vector3 spawnPos = new Vector3(
                            centerX,
                            prefabMC.MachineHeight,
                            centerZ
                        );
                        Quaternion spawnRot = Quaternion.Euler(prefabMC.InitialRotation);

                        GameObject slotMachine = Instantiate(
                            selectedPrefab,
                            spawnPos,
                            spawnRot,
                            slotMachinesParent.transform
                        );

                        Debug.Log($"Instantiated {slotMachine.name} at {spawnPos} with rotation {spawnRot.eulerAngles}");
